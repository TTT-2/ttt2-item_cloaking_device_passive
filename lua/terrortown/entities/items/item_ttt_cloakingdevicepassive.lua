...
ITEM.material = "vgui/ttt/icon_cloakingdevicepassive.vtm"
ITEM.CanBuy = {ROLE_TRAITOR, ROLE_DETECTIVE}

function ITEM:DrawInfo()                                    -- Sets the timer on the hud
	...
	if LocalPlayer():GetNWBool("cloakingdeviceready", false) then timeleft = "ready"
    elseif duration_end > math.Round(CurTime(), 1) then timeleft = math.Round(duration_end - CurTime())
    elseif cooldown_end > math.Round(CurTime(), 1) then timeleft = math.Round(-cooldown_end + CurTime())
    else timeleft = "ERROR" end
    return timeleft
end

if CLIENT then
	bind.Register("cloakingdevice", function()              -- Adds bind to the F1 Menu
